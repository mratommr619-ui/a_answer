...
def get_rotated_model():
    ...
    # Model Path ကို အတိအကျ models/ နဲ့ စပေးထားပါတယ်
...
<head>
    ...
    <style>
        ...
        .msg { position: relative; margin-bottom: 25px; padding: 15px; border-radius: 20px; font-size: 14px; max-width: 85%; line-height:1.6; }
...
<body>
    <div class="container">
        <div id="auth-screen" class="screen active" style="justify-content:center; text-align:center;">
            ...
            <div id="r-f" style="display:none">
                <input type="text" id="ru" placeholder="Choose Username" onkeypress="if(event.key==='Enter') auth('register')">
                <input type="password" id="rp" placeholder="Choose Password" onkeypress="if(event.key==='Enter') auth('register')">
    ...
    <script>
        ...
        async function ask(){
            ...
            } else target.innerHTML=`<span style="color:red">${d.error}</span><br><a href="https://t.me/mratom_619" style="color:var(--gold); font-size:10px;">Get Premium</a>`;
            box.scrollTop=box.scrollHeight;
        }
