...
<head>
    ...
    <style>
        ...
        .msg { max-width: 90%; padding: 15px; border-radius: 18px; font-size: 15px; line-height: 1.6; word-wrap: break-word; color: #fff !important; }
        .user { align-self: flex-end; background: linear-gradient(145deg, #d4af37, #f1c40f); color: #000 !important; font-weight: bold; }
        .bot { align-self: flex-start; background: #2a2a2a; border: 1px solid #333; box-shadow: 5px 5px 15px #0d0d0d; color: #fff !important; }
        ...
        input { flex: 1; padding: 15px; border-radius: 30px; border: none; background: #1a1a1a; color: #fff; outline: none; box-shadow: inset 2px 2px 5px #000; }
    ...
    <script>
        ...
        async function ask() {
            ...
            try {
                ...
                } else { target.innerText = "Error: " + (d.error || "No response"); }
...
async def ask_gemini(q: str):
    async with async_playwright() as p:
        # Persistent context သုံးပြီး login state ကို folder ထဲ သိမ်းမယ်
        ...
        try:
            ...
            for _ in range(20):
                ...
                if res:
                    text_el = await res[-1].query_selector(".model-response-text, div.markdown")
                    if text_el:
