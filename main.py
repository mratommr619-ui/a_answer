...
<head>
    ...
    <style>
        ...
        .container { width: 100%; max-width: 500px; background: var(--card); display: flex; flex-direction: column; box-shadow: 0 0 20px rgba(0,0,0,0.5); }
        ...
        pre { background: #000 !important; border-radius: 10px; padding: 10px; overflow-x: auto; border: 1px solid #444; margin: 10px 0; }
...
<body>
    ...
    <script>
        ...
        function appendMsg(q, a, i){
            ...
            let botHtml = `<div class="msg bot" id="${tid}">${formatAI(a)}</div>`;
            box.innerHTML += botHtml;
            const target = document.getElementById(tid);
            if(i) target.innerHTML += `<div style="margin-top:10px"><img src="${i}" style="width:100%; border-radius:10px; border:1px solid #444;" onclick="openI('${i}')"><a href="${i}" download target="_blank"><button style="width:100%; margin-top:8px; padding:10px; font-size:12px; background:#222; color:var(--gold);"><i class="fas fa-download"></i> DOWNLOAD IMAGE</button></a></div>`;
...
async def ask(data: dict):
    ...
    if any(w in q.lower() for w in ["draw", "image", "photo", "ပုံဆွဲ", "ရုပ်ပုံ"]):
        ...
        en_gen = client_tmp.chat.completions.create(
            ...
            messages=[{"role": "system", "content": "Convert the request into a single English image prompt. Output ONLY the prompt."},
                      {"role": "user", "content": q}]
        )
        ...
        answer_en = f"I have generated the image for: **{en_prompt}**"
    ...
    # --- ၂။ Text Logic ---
    sys_prompt = f"You are a wise Myanmar Professor. Be professional and natural. Reply in the user's language. \nChat History:\n{history_text}"
    try:
        client = get_rotated_groq_client()
        resp = client.chat.completions.create(
            model="llama-3.3-70b-versatile",
            messages=[{"role": "system", "content": sys_prompt},
                      {"role": "user", "content": f"{q} (If user speaks Myanmar, reply in professional English first, I will translate it later.) {yt_context}"}]
        )
