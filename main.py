...
# Secrets ထဲမှ Email နှင့် Pass ကို ယူခြင်း
...
<head>
    ...
    <style>
        :root {
            --bg-color: #1a1a1a; --container-bg: #222222;
            --gold-light: #f1c40f; --gold-dark: #d4af37;
            --shadow-out: 8px 8px 16px #0d0d0d, -8px -8px 16px #272727;
            --shadow-in: inset 6px 6px 12px #0d0d0d, inset -6px -6px 12px #272727;
        }
        html, body { margin: 0; padding: 0; width: 100%; height: 100%; overflow: hidden; background: var(--bg-color); font-family: sans-serif; color: #ffffff !important; }
        .main-wrapper { display: flex; flex-direction: column; width: 100%; height: 100vh; max-width: 600px; margin: 0 auto; background: var(--container-bg); }
        ...
        #chat-box { flex: 1; overflow-y: auto; padding: 20px; display: flex; flex-direction: column; gap: 15px; background: var(--container-bg); box-shadow: var(--shadow-in); margin: 10px; border-radius: 20px; }
        .msg { max-width: 90%; padding: 15px; border-radius: 18px; font-size: 15px; line-height: 1.6; word-wrap: break-word; animation: fadeIn 0.3s ease; }
        ...
        .bot { align-self: flex-start; background: #2a2a2a; border: 1px solid #333; color: #ffffff !important; box-shadow: var(--shadow-out); }
        ...
        .img-card { position: relative; border-radius: 12px; overflow: hidden; border: 1px solid var(--gold-dark); margin-top: 10px; }
...
<body>
    <div class="main-wrapper">
        ...
        <div class="input-container">
            <input type="text" id="userInput" placeholder="မေးခွန်းရိုက်ပါ..." autocomplete="off">
        ...
        async function ask() {
            ...
            try {
                ...
                if (d.answer) {
                    ...
                    cpBtn.className = 'icon-btn'; cpBtn.innerText = 'Copy Text';
                    cpBtn.onclick = () => { navigator.clipboard.writeText(d.answer); cpBtn.innerText = '✅ Copied!'; };
                    tools.appendChild(cpBtn); target.appendChild(tools);
                }
                if (d.images && d.images.length > 0) {
                    d.images.forEach(src => {
                        target.innerHTML += `<div class="img-card"><img src="${src}" class="gemini-img"><button class="dl-btn" onclick="window.open('${src}', '_blank')">DL</button></div>`;
                    });
                }
...
async def ask_gemini(q: str):
    async with async_playwright() as p:
        ...
        try:
            # persistent context သုံးပြီး login ကို folder ထဲမှာ သိမ်းခိုင်းမယ်
            ...
            # Login Button တွေ့လျှင် Login ဝင်မည့် Logic
            if await page.query_selector("text=Sign in"):
                ...
                # 2FA အတွက် စက္ကန့်အနည်းငယ် စောင့်ခြင်း
